...
export default function CardDetails() {
  // 👇 ahora leemos también el id dinámico
  ...
  // gradiente para la tarjeta (array de colores desde theme)
  const gradient = CARD_GRADS[account] as ReadonlyArray<string>;
  const accountColor = ACCOUNT_PRIMARY[account];

  // Pequeño mapping para mostrar un título bonito según el id
  const cardTitle =
    cardId.toLowerCase() === "general"
      ? "General"
      : cardId.charAt(0).toUpperCase() + cardId.slice(1);

  // alias de ejemplo combinando account + id
...
function CardFaceSVG({
  ...
  const c0 = gradient[0] ?? "#000";
  const c1 = gradient[1] ?? c0;
  ...
  return (
    <View style={styles.card}>
      ...
      <Svg width="100%" height="100%" style={StyleSheet.absoluteFill}>
        <Defs>
          <SvgLG id="cardGrad" x1="0" y1="0" x2="1" y2="1">
            <Stop offset="0" stopColor={c0} />
            <Stop offset="1" stopColor={c1} />
          </SvgLG>
        </Defs>
        <Rect width="100%" height="100%" rx={22} fill="url(#cardGrad)" />
      </Svg>
...
const styles = StyleSheet.create({
  ...
  cardTopRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 18, paddingTop: 18 },
