...
export default function SendAmount() {
  ...
  const tokenSym = useMemo(
    () => tokenId.split(".")[0]?.toUpperCase() || "USD",
    [tokenId]
  );
  ...
  const next = () => {
    ...
    const q = new URLSearchParams({
      ...
      to,
      account,
      ...(network ? { network } : {}), // 👈 propaga la red si venía
    });
    router.push(`/(tabs)/send/confirm?${q.toString()}`);
  };

  return (
    <GlassScaffold title="Send" account={account} wrapInGlass>
      {/* To */}
      ...
      <View style={s.inputRow}>
        ...
        <Pressable
          ...
          accessibilityLabel="Search a friend"
        >
